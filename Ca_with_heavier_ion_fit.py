...
def find_and_rename_images(base_folder):  
    ...
    for item in os.listdir(base_folder):
        ...
        def Fit(x_data , y_data):
            def F(params, x, y):
                a, h, v, u, w = params
                inside = np.maximum(1 - ((y - w) / v)**2, 0)
                sqrt_term = np.sqrt(inside)
                return (((x - u)/h - a/h - sqrt_term) *
                        ((x - u)/h + a/h + sqrt_term) *
                        ((y - w)/v - 1) *
                        ((y - w)/v + 1))
            ...
            def initial_parameters(x , y):
                ...
                def a_h(x , y , v , w , L):
                    ...
                    flat_upper = y > w + (1 - y_fraction) * v
                    flat_lower = y < w - (1 - y_fraction) * v
                    ...
                    # finding length of flat part
                    x_flat_diff = np.max(x_flat) - np.min(x_flat)
                ...
                v = (np.max(y) - np.min(y)) / 2
                a , h = a_h(x , y , v , w , L)
                
                init = np.array([a, h, v, u, w])
                
                lower = np.array([0.5*a , 0.5*h , 0.5*v , 0.9*u , 0.9*w])
                upper = np.array([1.5*a , 1.5*h , 1.5*v , 1.1*u , 1.1*w])
        ...
        for file in os.listdir(item_path):
            ...
            if file == 'trap.info':
                ...
                plt.hlines(w + v, x2[-1], x1[-1], colors='r', linestyles='-')
                plt.hlines(w - v, x2[0], x1[0], colors='r', linestyles='-')
